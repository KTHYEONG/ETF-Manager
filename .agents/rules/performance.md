...
trigger:
  ...
  - on_file_path_regex: "src/.*(sim|data|features|validation|execution|portfolio|rebalance).*"
  - on_file_path_glob: ["src/sim/**/*.py", "src/data/**/*.py", "src/features/**/*.py", "src/portfolio/**/*.py", "src/rebalance/**/*.py"]
priority: 10
---
...
- **Escalate When Justified:** If a measured hot path cannot be efficiently handled with standard vectorization (e.g., long-horizon simulation paths, dynamic rebalance loops), evaluate JIT compilation, native code, out-of-core engines, or parallelism based on the bottleneck shape. Adopt added complexity only when benchmarks demonstrate meaningful gains.
