...
trigger:
  - on_label: ["logging"]
  - on_file_path_regex: "src/.*(logging|engine|execution|api|pipeline|service|orchestration|portfolio|rebalance).*"
  - on_file_path_glob: ["src/**/logging/**/*.py", "src/**/portfolio/**/*.py", "src/**/rebalance/**/*.py"]
priority: 9
---
...
- **Precision Preservation:** Format numbers compactly, but retain sufficient precision to diagnose the underlying quantity (e.g., fee rates, tracking error, expense ratios, optimizer tolerances, and weights must not be rounded into meaningless zeros).
- **Correlation Identifiers:** Include stable identifiers (e.g., `run_id`, `rebalance_id`, `trial_id`, `symbol`, or `stage`) when concurrent or multi-stage pipelines make event attribution ambiguous.
...
- **Operational vs. High-Frequency Logs:** Keep high-frequency streaming data (e.g., tick feeds, intra-second iNAV updates, raw optimizer trials, detailed memory snapshots) isolated in dedicated log destinations (e.g., specific `.jsonl` or diagnostic artifacts) rather than spamming primary system logs.
- **INFO vs. DEBUG/TRACE:**
