...
from src.policy.tilt import FactorTilt, apply_fixed_tilt, resolve_tilted_targets
...
def test_pol_h03_none_tilt_is_identity(scenario_id: str) -> None:
    ...
    assert resolve_tilted_targets(
        PolicyId.WORLD_SPLIT, prices, prices, _SIGNAL_AT, None
    ) == resolve_targets(PolicyId.WORLD_SPLIT, prices, _SIGNAL_AT)
