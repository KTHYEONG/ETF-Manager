...
def test_preload_hp_context_reads_pinned_snapshot(tmp_path, monkeypatch) -> None:
    """One verified snapshot feeds every PIT input of the HP allocation context."""
    from datetime import UTC, date, datetime

    import polars as pl

    from src.analytics.adaptive_hp_screen import preload_hp_allocation_context
    ...
    persist_ingest(
        pl.DataFrame(
            {
                "ticker": ["QQQ"] * len(sessions), "date": sessions,
    ...
    context = preload_hp_allocation_context(settings, start=date(2024, 1, 15), end=date(2024, 2, 26))
