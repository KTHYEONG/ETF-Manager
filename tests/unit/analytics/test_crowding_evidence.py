...
@pytest.mark.parametrize("scenario_id", ["test_compute_crowding_slot_physical_automation_botz"])
def test_compute_crowding_slot_physical_automation_botz(
    scenario_id: str, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """test_compute_crowding_slot_physical_automation_botz"""
    from src.analytics.crowding_evidence import compute_crowding_slot
    from src.data.pit import stamp_availability
    from src.data.schema import Dataset, spec_for
    from src.data.settings import DataSettings
