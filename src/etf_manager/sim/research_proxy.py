...
def run_research_proxy(
    ...
    Raises:
        ValueError: When the policy is not R1_US_MKT_FF, costs are nonzero, or the
    ...
    """
    if config.policy is not PolicyId.R1_US_MKT_FF:
        raise ValueError(f"run_research_proxy requires R1_US_MKT_FF, got {config.policy!s}")
