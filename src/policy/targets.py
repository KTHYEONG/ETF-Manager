...
POLICY_ALIASES: Final[Mapping[str, PolicyId]] = {
    ...
    # Legacy research-wave strings keep parsing so old notes and ADR loaders stay valid.
    "global": PolicyId.VT,
    "s0_global": PolicyId.VT,
    "us": PolicyId.VTI,
    "s1_us": PolicyId.VTI,
    "regional": PolicyId.WORLD_SPLIT,
    "s2_regional": PolicyId.WORLD_SPLIT,
    "global_bond": PolicyId.VT_BND,
    "s3_global_bond": PolicyId.VT_BND,
    "defensive": PolicyId.VT_TREAS,
    "s4_defensive": PolicyId.VT_TREAS,
    "s5_invvol": PolicyId.INV_VOL,
    "us_value": PolicyId.VTI_VTV,
    "s6_us_core_value": PolicyId.VTI_VTV,
    "us_large": PolicyId.IVV,
    "s7_us_large_cap": PolicyId.IVV,
    "nasdaq": PolicyId.QQQ,
    "s8_us_nasdaq": PolicyId.QQQ,
    "us_ff": PolicyId.FF_PROXY,
    "r1_us_mkt_ff": PolicyId.FF_PROXY,
}
...
class UsEquityUniverse(StrEnum):
    ...
    # PolicyId.QQQ resolves this bucket through UNIVERSE_VEHICLE into its single sleeve.
...
_STATIC_TARGETS: Final[dict[PolicyId, dict[str, float]]] = {
    PolicyId.VT: {"VT": 1.0},
    PolicyId.VTI: {UNIVERSE_VEHICLE[UsEquityUniverse.TOTAL_MARKET]: 1.0},
    PolicyId.WORLD_SPLIT: {"VTI": 0.5, "VEA": 0.3, "VWO": 0.2},
    PolicyId.VT_BND: {"VT": 0.7, "BND": 0.3},
    PolicyId.VT_TREAS: {"VT": 0.6, "IEF": 0.2, "TLT": 0.2},
    PolicyId.VTI_VTV: {"VTI": 0.8, "VTV": 0.2},
    PolicyId.IVV: {UNIVERSE_VEHICLE[UsEquityUniverse.LARGE_CAP]: 1.0},
    PolicyId.QQQ: {UNIVERSE_VEHICLE[UsEquityUniverse.NASDAQ_100]: 1.0},
}
...
def resolve_targets(policy: PolicyId, prices: pl.DataFrame, signal_at: datetime) -> dict[str, float]:
    ...
    Static ids ignore ``prices`` entirely; ``INV_VOL`` weighs each regional sleeve
    ...
    Raises:
        ...
        PolicyError: When the requested policy is a research_proxy identity, or any
            ...
            simplex invariants.
    """
    if policy is PolicyId.FF_PROXY:
        raise PolicyError("FF_PROXY is a research_proxy identity; it has no ETF target weights")
    if signal_at.tzinfo is None:
        raise ValueError(f"signal_at must be timezone-aware, got naive datetime {signal_at!r}")
    targets = _invvol_targets(prices, signal_at) if policy is PolicyId.INV_VOL else dict(_STATIC_TARGETS[policy])
