# Plans

Implementation roadmaps and architecture evolution notes. These documents explain **why** and **in what order**; normative contracts live in `docs/architecture/`.

| Document | Purpose |
| --- | --- |
| [v2_thesis_evolution.md](v2_thesis_evolution.md) | Post-kernel v2 roadmap: integrity → evidence meaning → PIT refresh → AI Compute slice → Power → Automation |

Kernel (old Waves 0–7: identity, experiments, holdings, evidence blob, horizon, prospective, regime proxy, thesis-wave E2E) is **first-pass complete**. Next implementation is **Wave A (integrity)** in that plan.

Review input for the rewrite: `docs/feedback.md` (2026-08-29). Results: `docs/results/20260829_v2_thesis_wave_detail.md`.
